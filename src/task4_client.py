...
class search_action_client(object):
    ...
    def main(self):
        while not self.ctrl_c:
            self.send_goal(velocity = 0.25, approach = 0.25)
            prempt = False
            while self.client.get_state() < 2:
                #print(f"FEEDBACK: Currently travelled {self.distance:.3f} m, " f"STATE: Current state code is {self.client.get_state()}")
                if self.distance >= 5:
            ...
            #print(f"RESULT: closest object {result.closest_object_distance:.3f} m away " f"at a location of {result.closest_object_angle:.3f} degrees")

if __name__ == '__main__':
    client_instance = search_action_client()
    try:
